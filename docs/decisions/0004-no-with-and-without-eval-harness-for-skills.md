...
input's HTML and React wrappers posting two different server contracts, and the
documented `pnpm add` resolving to npm's `latest` tag — which for
`@govtech-bb/frontend` is four releases stale, and for `@govtech-bb/react` is a
build of the _previous_ Tailwind and Radix design system. None of it came from
a score.
